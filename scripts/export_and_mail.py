...
def score_gouv_result(
    ...
    company_n = normalize_company_name(company_raw)
    ...
    for token in [brand_email, brand_web, company_n]:
        if token and token in denom_n:
            score += 20

    if company_n and denom_n == company_n:
        score += 40
    elif company_n and (company_n in denom_n or denom_n in company_n):
        score += 15
...
def search_gouv_company_ranked(company_raw: str, email: str, website: str, city: str, ocr_text: str,
    ...
    for q in candidates:
        ...
        for e in results:
            score = score_gouv_result(
                e=e,
                company_raw=company_raw or q,
                email=email,
                website=website,
                city=city,
            ...
            if score > best_score:
                ...
                best = {
                    "name": (e.get("nom_raison_sociale") or e.get("denomination") or "").strip(),
                    "siret": validate_siret(siege.get("siret") or ""),
                    "naf": naf,
                    "address": (siege.get("adresse") or siege.get("libelle_voie") or ""),
                    "postal_code": normalize_cp(siege.get("code_postal") or ""),
                    "city": (siege.get("libelle_commune") or "").strip(),
                    "dirigeant": dirigeant,
                    "_score": score,
                }
...
def record_score(d: Dict[str, Any]) -> int:
    ...
    score = sum(1 for k in fields if str(d.get(k, "") or "").strip() != "")
    if str(d.get("email", "")).strip():
        score += 2
    if str(d.get("phone", "")).strip():
        score += 2
    if str(d.get("interlocuteur", "")).strip():
        score += 2
    return score
...
def normalize_row_core(d: Dict[str, Any]) -> Dict[str, Any]:
    d["name"] = re.sub(r"\s+", " ", (d.get("name") or "").strip())
    d["address"] = re.sub(r"\s+", " ", (d.get("address") or "").strip())
    d["postal_code"] = normalize_cp(d.get("postal_code") or "")
    d["city"] = re.sub(r"\s+", " ", (d.get("city") or "").strip())
    d["siret"] = validate_siret(d.get("siret", "") or "")
    d["naf"] = re.sub(r"[^A-Za-z0-9]", "", (d.get("naf") or "").upper())
    d["phone"] = normalize_phone(d.get("phone", "") or "")
    d["phone2"] = normalize_phone(d.get("phone2", "") or "")
    if d["phone"] and d["phone2"] and d["phone"] == d["phone2"]:
        d["phone2"] = ""
    d["email"] = clean_email(d.get("email", "")) if is_valid_email(d.get("email", "")) else ""
    if not d.get("website") and d["email"]:
        dom = domain_from_email(d["email"])
        if dom:
            d["website"] = f"https://{dom}"
...
def merge_prefer_card(base: Dict[str, Any], card_row: Dict[str, Any]) -> Dict[str, Any]:
    out = dict(base)

    prefer_fields = [
        "interlocuteur", "contact_firstname", "contact_lastname",
        "phone", "phone2", "email", "website", "address", "postal_code", "city"
    ]
    for f in prefer_fields:
        card_val = str(card_row.get(f, "") or "").strip()
        if card_val:
            out[f] = card_val

    if str(card_row.get("name", "")).strip() and not str(out.get("name", "")).strip():
        out["name"] = card_row["name"]

    if str(card_row.get("activity_summary", "")).strip():
        out["activity_summary"] = card_row["activity_summary"]

    out = normalize_row_core(out)
    return out
...
def unify_from_prospect(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    d = {
        ...
        "_card_photo_file_id": p.get("card_photo_file_id", ""),
        "_card_kv_key": p.get("card_kv_key", ""),
        "_source_record": p,
    }
    d = normalize_row_core(d)
    return d


def build_card_pipeline_row(date: str, agency: str, initials: str, comment: str, img_bytes: bytes,
                            forced_company: str = "", forced_city: str = "", base_record: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    base = {
    ...
    if not img_bytes:
        return normalize_row_core(base)

    img_hash = image_hash_bytes(img_bytes)
    if img_hash in CARD_PIPELINE_CACHE:
        row = dict(CARD_PIPELINE_CACHE[img_hash])
        row["date"] = date
        row["agency"] = agency
        row["initials"] = initials
        row["resume"] = (comment or "").strip()
        row["commande"] = (base_record or {}).get("commande", "")
        row["dirigeant"] = (base_record or {}).get("dirigeant", row.get("dirigeant", ""))
        return normalize_row_core(row)
    ...
    if not local_website and local_email:
        dom = domain_from_email(local_email)
        if dom:
            local_website = f"https://{dom}"

    detected_company_line = detect_company_from_lines(
        lines,
        gemini_company=(vis.get("company") or forced_company or "").strip(),
    ...
    local_city = (vis.get("city") or forced_city or local_city_ocr or "").strip()
    ...
    place = {}
    place_query_name = (gouv.get("name") or company_guess or forced_company or "").strip()
    if place_query_name:
    ...
    final_dirigeant = (base_record or {}).get("dirigeant") or (gouv.get("dirigeant") or "").strip()
    ...
    row.update({
        ...
        "activity_summary": enrich_activity_summary(
            ...
            company_hint=(company_guess or detected_company_line or forced_company or "")
        ),
        ...
        "email": final_email,
        "website": final_website,
        "commande": (base_record or {}).get("commande", ""),
    })

    row = normalize_row_core(row)
    CARD_PIPELINE_CACHE[img_hash] = dict(row)
    return row


def unify_from_card(date: str, agency: str, initials: str, comment: str, img_bytes: bytes) -> Dict[str, Any]:
    return build_card_pipeline_row(date, agency, initials, comment, img_bytes)
...
def unify_from_photo(date: str, agency: str, initials: str, city_hint: str, comment: str, img_bytes: bytes) -> Dict[str, Any]:
    ...
    if not website and email_ocr:
        dom = domain_from_email(email_ocr)
        if dom:
            website = f"https://{dom}"
...
def resolve_linked_card_record_for_prospect(prospect: Dict[str, Any], all_cards: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    card_key = str(prospect.get("card_kv_key") or "").strip()
    card_file_id = str(prospect.get("card_photo_file_id") or "").strip()
    p_key = str(prospect.get("_kv_key") or "").strip()

    if card_key:
        for c in all_cards:
            if str(c.get("_kv_key") or "").strip() == card_key:
                return c

    if p_key:
        for c in all_cards:
            if str(c.get("linked_prospect_key") or "").strip() == p_key:
                return c

    if card_file_id:
        for c in all_cards:
            if str(c.get("file_id") or "").strip() == card_file_id:
                return c

    return None


def build_rows_with_linked_cards(
    date: str,
    prospects: List[Dict[str, Any]],
    photos: List[Dict[str, Any]],
    cards: List[Dict[str, Any]],
    agency_filter: Optional[str] = None,
    initials_filter: Optional[str] = None,
) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    consumed_card_keys = set()

    def agency_ok(v: str) -> bool:
        return not agency_filter or (v or "").upper() == agency_filter

    def initials_ok(v: str) -> bool:
        return not initials_filter or (v or "").upper() == initials_filter

    scoped_prospects = []
    for p in prospects:
        ag = (p.get("agency") or "").upper()
        ini = (p.get("initials") or "").upper()
        if agency_ok(ag) and initials_ok(ini):
            p2 = dict(p)
            p2["_kv_key"] = p.get("_kv_key", "")
            scoped_prospects.append(p2)

    scoped_photos = []
    for ph in photos:
        ag = (ph.get("agency") or "").upper()
        ini = (ph.get("user") or ph.get("initials") or "").upper()
        if agency_ok(ag) and initials_ok(ini):
            scoped_photos.append(ph)

    scoped_cards = []
    for ca in cards:
        ag = (ca.get("agency") or "").upper()
        ini = (ca.get("user") or ca.get("initials") or "").upper()
        if agency_ok(ag) and initials_ok(ini):
            scoped_cards.append(ca)

    for p in scoped_prospects:
        base_row = unify_from_prospect(p)
        linked_card = resolve_linked_card_record_for_prospect(p, scoped_cards)

        if linked_card:
            fid = linked_card.get("file_id") or ""
            try:
            ...
            if img:
                card_row = build_card_pipeline_row(
                    date=date,
                    agency=base_row["agency"],
                    initials=base_row["initials"],
                    comment=linked_card.get("comment") or "",
                    img_bytes=img,
                    forced_company=base_row.get("name", ""),
                    forced_city=base_row.get("city", ""),
                    base_record=base_row,
                )
                merged = merge_prefer_card(base_row, card_row)
                rows.append(merged)
                consumed_card_keys.add(str(linked_card.get("_kv_key") or ""))
                continue

        rows.append(base_row)

    for ph in scoped_photos[:MAX_PHOTO_IMAGES if initials_filter else 400]:
        fid = ph.get("file_id") or ""
        city_hint = ph.get("city") or ""
        comment = ph.get("comment") or ph.get("meeting") or ""
        agency = (ph.get("agency") or "").upper()
        initials = (ph.get("user") or ph.get("initials") or "").upper()
        try:
            img = tg_download_file(fid) if fid else b""
        except Exception:
            img = b""
    ...
    max_cards = MAX_OCR_IMAGES if initials_filter else 600
    for ca in scoped_cards[:max_cards]:
        if str(ca.get("_kv_key") or "") in consumed_card_keys:
            continue
        if str(ca.get("source_mode") or "").strip() == "company_form" and str(ca.get("status") or "").strip() == "linked":
            continue

        fid = ca.get("file_id") or ""
        comment = ca.get("comment") or ""
        agency = (ca.get("agency") or "").upper()
        initials = (ca.get("user") or ca.get("initials") or "").upper()
        try:
            img = tg_download_file(fid) if fid else b""
        except Exception:
            img = b""
    ...
    rows = [normalize_row_core(r) for r in rows if any(str(v or "").strip() for v in r.values())]
    return rows
