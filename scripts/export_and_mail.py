...
def build_excel(records):
    ...
    for r in records:
        infos_com = r.get("infos_commerciales", "") or " | ".join([x for x in [
...
def main():
    ...
    for r in prospects:
        ...
        if file_id:
            ...
            if not fp:
                print(f"[CARD][SKIP] getFile failed file_id={file_id[:20]}...")
            else:
                img_bytes = tg_download_file(telegram_token, fp)
                if not img_bytes:
                    print(f"[CARD][SKIP] download failed fp={fp}")
                else:
                    try:
    ...
    for ag, recs in agency_records.items():
        ...
        for i, r in enumerate(recs, 1):
            ...
            fp = tg_api_get_file(telegram_token, file_id)
            if not fp:
                print(f"[CARD][SKIP] getFile failed | agency={ag} | file_id={file_id[:20]}...")
                continue
            img = tg_download_file(telegram_token, fp)
            if not img:
                print(f"[CARD][SKIP] download failed | agency={ag} | fp={fp}")
                continue
